...
private:
  ...
  void nearestUtil(KDNode *root, const Point &point, unsigned depth,
    ...
    // Point has the x and y values, rewrite the above line to use the x and y
    // values
    if (cd == 0) {
    ...
    // if ((point[cd] - root->point[cd]) * (point[cd] - root->point[cd]) <
    // bestDist) {
