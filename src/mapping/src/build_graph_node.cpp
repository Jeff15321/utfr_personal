...
void BuildGraphNode::stateEstimationCB(const utfr_msgs::msg::EgoState msg) {
  ...
  g2o::VertexSE2 *poseVertex = createPoseNode(
      current_pose_id_, msg.pose.pose.position.x, msg.pose.pose.position.y,
      utfr_dv::util::quaternionToYaw(msg.pose.pose.orientation));
  ...
  g2o::EdgeSE2 *edge = addPoseToPoseEdge(id_to_pose_map_[current_pose_id_ - 1],
                                         poseVertex, dx, dy, dtheta, false);
    ...
    for (int coneID : cones) {
      ...
      if (landmarked_ == true && out_of_frame_ >= 1000) {
        ...
        if (seen_status != cones.end()) {
          ...
          g2o::VertexSE2 *first_pose_node =
              id_to_pose_map_[first_detection_pose_id_];
          g2o::VertexSE2 *second_pose_node = id_to_pose_map_[current_pose_id_];

          // add an edge using the pose ids at initial detection and loop
          // closure detection
          g2o::EdgeSE2 *edge = addPoseToPoseEdge(
              first_pose_node, second_pose_node, dx, dy, dtheta, loop_closed_);
          pose_to_pose_edges_.push_back(edge);
...
g2o::VertexPointXY *BuildGraphNode::createConeVertex(int id, double x,
                                                     double y) {
...
g2o::EdgeSE2 *BuildGraphNode::addPoseToPoseEdge(g2o::VertexSE2 *pose1,
                                                g2o::VertexSE2 *pose2,
                                                double dx, double dy,
                                                double dtheta,
                                                bool loop_closure) {
