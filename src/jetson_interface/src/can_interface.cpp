...
int CanInterface::get_can(msg_dvjet_sensor_e msgName) {
    ...
    if(msgName == msg_dvjet_sensor_e::ANGSENREC){
        result =  (int)((messages[msg_array_dvjet_sensor[(int)msgName]].data[2]) | 
                    (((messages[msg_array_dvjet_sensor[(int)msgName]].data[3])<<8)));   
    }
    else if(msgName == msg_dvjet_sensor_e::MOTPOS){
      result = (int)((messages[msg_array_dvjet_sensor[(int)msgName]].data[0]) | 
                    (((messages[msg_array_dvjet_sensor[(int)msgName]].data[1])<<8))); 
    }
    else {
        result =  ARRAY_TO_INT64(messages[msg_array_dvjet_sensor[(int)msgName]].data); //messages[msg_array_dvjet_sensor[(int)msgName]].data;
    }
...
static void* thread_read(void* node){
    utfr_dv::jetson_interface::CanInterface* canNode = (utfr_dv::jetson_interface::CanInterface*) node;
