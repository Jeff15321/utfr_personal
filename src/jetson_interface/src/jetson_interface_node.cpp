...
void JetsonInterface::initSubscribers(){
  control_cmd_subscriber_ = this->create_subscription<utfr_msgs::msg::ControlCmd>(
    topics::kControlCmd, 10, 
    std::bind(&JetsonInterface::controlCmdCB, this, _1));
  system_status_subscriber_ = this->create_subscription<utfr_msgs::msg::SystemStatus>(
...
void JetsonInterface::initPublishers(){
  jetson_publisher_ = this->create_publisher<utfr_msgs::msg::SensorCan>(
    topics::kJetson, 10);
  system_status_publisher_ = this->create_publisher<utfr_msgs::msg::SystemStatus>(
...
void JetsonInterface::controlCmdCB(const utfr_msgs::msg::ControlCmd& msg){
  ...
  RCLCPP_INFO(this->get_logger(),"Sending Throttle: %f", (double)(int)((throttle_cmd_)));

  can1_->write_can(msg_dvjet_sensor_e::DV_THR_COMMAND, (double)(((throttle_cmd_<<1)|0x1)));
}
...
void JetsonInterface::timerCB(){
  ...
  try {
    ...
    //*****   Steering   *****
    //Get steering angle:
    current_steering_angle_ = -((int16_t)(can1_->get_can(msg_dvjet_sensor_e::ANGSENREC))/10);

    //Check for sensor malfunction:
    if(current_steering_angle_!=-3276){
      jetson_cmd_.steering_angle = current_steering_angle_;
      RCLCPP_INFO(this->get_logger(),"Steer Angle: %d", current_steering_angle_);
    }else{
    ...
    long dv_driving_dynamics_1 = can1->get_can(msg_dvjet_sensor_e::DVDrivingDynamics1);

    as_state_.speed_actual = (uint8_t)dv_driving_dynamics_1&0xFF;
    as_state_.speed_target = (uint8_t)(dv_driving_dynamics_1<<(8))&0xFF;

    as_state_.steering_angle_actual = (uint8_t)((dv_driving_dynamics_1<<(8*2))&0xFF);
    as_state_.steering_angle_target = (uint8_t)((dv_driving_dynamics_1<<(8*3))&0xFF);

    as_state_.brake_hydr_actual = (uint8_t)((dv_driving_dynamics_1<<(8*4))&0xFF);
    as_state_.brake_hydr_target = (uint8_t)((dv_driving_dynamics_1<<(8*5))&0xFF);

    as_state_.motor_moment_actual = (uint8_t)((dv_driving_dynamics_1<<(8*6))&0xFF);
    as_state_.motor_moment_target = (uint8_t)((dv_driving_dynamics_1<<(8*7))&0xFF);

    //Dv driving dynamics 2:
    long dv_driving_dynamics_2 = can1->get_can(msg_dvjet_sensor_e::DVDrivingDynamics2);

    as_state_.acceleration_longitudinal = (int16_t)dv_driving_dynamics_2&0xFFFF;
    as_state_.acceleration_lateral = (int16_t)(dv_driving_dynamics_2<<(16))&0xFFFF;
    as_state_.yaw_rate = (int16_t)((dv_driving_dynamics_2<<(16*2))&0xFFFF);
