...
TEST(ClustererTest, RemoveGroundTest) {
    ...
    //     cout << filtered_points[i][0] << " " << filtered_points[i][1] << " " << filtered_points[i][2] << endl;
    // }
...
TEST(ClustererTest, ClusterTest) {
    ...
    //     cout << cluster_centers[i][0] << " " << cluster_centers[i][1] << " " << cluster_centers[i][2] << endl;
    // }
...
TEST(ClustererTest, ReconstructTest) {
    ...
    std::vector<PointCloud> clusters = clusterer.reconstruct(points, cluster_centers);
    
    // For debugging
    // for(int i = 0; i < clusters.size(); i++){
    //     for(int j = 0; j < clusters[i].size(); j++){
    //         cout << clusters[i][j][0] << " " << clusters[i][j][1] << " " << clusters[i][j][2] << endl;
    //     }
    // }
...
TEST(ClustererTest, CleanAndClusterTest) {
    ...
    Grid min_points_grid; // Assuming Grid is correctly defined and initialized
    std::tuple<PointCloud, std::vector<PointCloud>> output = clusterer.clean_and_cluster(points, min_points_grid);
