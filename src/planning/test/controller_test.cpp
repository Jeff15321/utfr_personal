...
TEST(ControllerNode, calculateVelocities) {
  ControllerNode node;
...
TEST(ControllerNode, filterVelocities) {
  ControllerNode node;
  std::vector<double> v;
...
TEST(ControllerNode, getMaxLongAccelGGV) {
  ControllerNode node;
  double velocity;
  double a_lateral;
  double a_longit;
  ...
  velocity = 10.4;
  a_lateral = 15.1;
  a_longit = node.getMaxLongAccelGGV(velocity, a_lateral);
