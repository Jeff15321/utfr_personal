  ...
  namespace center_path
  {
    ...
    void CenterPathNode::initEvent()
    {
      if (event_ == "read")
      {
        mission_subscriber_ = this->create_subscription<utfr_msgs::msg::SystemStatus>(
            topics::kSystemStatus, 10, std::bind(&CenterPathNode::missionCB, this, std::placeholders::_1));
      }
    }
    ...
    CenterPathNode::BezierPoints(
        std::vector<CGAL::Point_2<CGAL::Epick>>
            midpoints)
    { // Creating functions x(t) and y(t) given midpoints
      ...
      for (int i = 1; i <= degree;
           i++)
      { // std::min(bezier_points.size(), midpoints.size())
