...
def bounding_boxes_to_cone_detections(left_bounding_boxes, left_classes, \
right_bounding_boxes, right_classes, translation, intrinsics_left, left_img_, right_img_):
    ...
    for (x, y, w, h) in left_bounding_boxes:
      ...
      right_x, right_y, index = find_cone_other_frame_closest(x, y, right_bounding_boxes, \
                                              left_classes[i], right_classes, matched_array)
      matched_array.append(index)

      disparity = abs(right_x - x)
      
      
      depth = find_depth((right_x, right_y), (x, y), right_img_, left_img_, baseline, f)
...
def find_cone_other_frame_closest(left_cone_x, left_cone_y, right_detections, \
                                  left_class, right_classes, matched_array):
    curr_min_dist = 1000
    ...
    for (x, y, w, h) in right_detections:
      if (euclidean_distance(left_cone_x, x, left_cone_y, y)) < curr_min_dist and left_class == right_classes[i]\
        and i not in matched_array and x >= left_cone_x:
