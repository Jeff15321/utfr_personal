...
from .submodules.classical import classical_process
...
class PerceptionNode(Node):
  ...
  def loadParams(self):
    ...
    self.declare_parameter(\
    	'confidence', 0.70)
    self.declare_parameter(\
      'perception_debug_topic', "/perception/debug")
    ...
    self.distortion_left = self.get_parameter(
        'distortion_left').get_parameter_value().double_array_value
    self.distortion_right = self.get_parameter(
        'distortion_right').get_parameter_value().double_array_value
    self.intrinsics_left = self.get_parameter(
        'intrinsics_left').get_parameter_value().double_array_value
    self.intrinsics_right = self.get_parameter(
        'intrinsics_right').get_parameter_value().double_array_value
    self.rotation = self.get_parameter(
        'rotation').get_parameter_value().double_array_value
    self.translation = self.get_parameter(
        'translation').get_parameter_value().double_array_value
    self.rectify_left = self.get_parameter(
        'rectify_left').get_parameter_value().double_array_value
    self.rectify_right = self.get_parameter(
        'rectify_right').get_parameter_value().double_array_value
    self.perception_debug_topic_ = self.get_parameter(
        'perception_debug_topic').get_parameter_value().string_value
    self.save_pic  = self.get_parameter(
    	'save_pic').get_parameter_value().string_value
    self.confidence_ = self.get_parameter(
  ...
  def initVariables(self):
    ...
    self.session = ort.InferenceSession("src/perception/perception/best.onnx", providers=providers)
  ...
  def initSubscribers(self):
    ...
    self.left_cam_ready_subscriber_ = self.create_subscription(
        Bool,
        "/left_camera_node/ready",
        self.leftCameraReadyCB,
        qos_latching)

    self.right_cam_ready_subscriber_ = self.create_subscription(
        Bool,
        "/right_camera_node/ready",
        self.rightCameraReadyCB,
        qos_latching)
  ...
  def initPublishers(self):
    ...
    self.perception_debug_publisher_ = \
        self.create_publisher(PerceptionDebug, self.perception_debug_topic_, 1)
  ...
  """
    ...
    right_bounding_boxes, right_classes, right_image = deep_process(right_img_, \
      self.translation, self.intrinsics_left, self.session, self.confidence_)
    cone_detections, left_image, right_image = bounding_boxes_to_cone_detections(left_bounding_boxes, \
      left_classes, right_bounding_boxes, right_classes, self.translation, self.intrinsics_left\
  ...
  def save_image(self, left_img_, right_img_, rosbag_name):
    ...
    cv2.imwrite("/home/utfrdv/utfr_dv_ros2/src/perception/images_to_mp4_left/" + left_img_name, left_img_)

    right_img_name = rosbag_name + "_" + str(self.saved_count) + '.png'
    cv2.imwrite("/home/utfrdv/utfr_dv_ros2/src/perception/images_to_mp4_right/" + right_img_name, right_img_)
  ...
  def visualize_detections(self, frame_left, frame_right, results_left, results_right, cone_detections):
    i = 0
    ...
    for (x, y, w, h) in results_left:
      ...
      cv2.putText(frame_left, text="(" + str(round(x_3d, 2)) + ", " + str(round(y_3d, 2)) + ", " + str(round(z_3d, 2)) + ")", org=(int(x), int(y - 10)), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.5, color=(0, 0, 255), thickness=1, lineType=cv2.LINE_AA)        
      i += 1
  ...
  def cameraStatus(self):
    ...
    elif self.left_img_.shape[0] < self.MIN_HEIGHT or self.left_img_.shape[1] < self.MIN_WIDTH:
      return 2 #FATAL
