...
#        mountPointString = "GET %s HTTP/1.1\r\nUser-Agent: %s\r\n" % (self.mountpoint, useragent)
    ...
    def decode_rtcm(self, data):
        from pyubx2 import UBXReader
        from pynmeagps import NMEAReader
        from pyrtcm import RTCMReader, RTCMMessage

        decoded_data = RTCMReader.parse(data)
        return decoded_data
...
#                        print(casterResponse)
                    ...
                    while data:
                        try:
                            ...
                            # try:
                            #     decoded_data = self.decode_rtcm('b'+''.join(f'\\x{byte:02x}' for byte in data))
                            #     print(decoded_data)
                            # except:
                            #     print("Failure")
                            # decoded_data = self.decode_rtcm(data)
                            # print(decoded_data)
